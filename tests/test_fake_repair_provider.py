...
def test_fake_provider_proposes_parking_strip_patch_and_manual_revalidation_passes() -> None:
    ...
    failed = _failed_check(invalid, "parked_van_footprint_in_parking_strip")
    ...
    assert _check(repaired, "parked_van_footprint_in_parking_strip").passed is True
...
def test_fake_provider_proposes_trigger_patch_and_manual_revalidation_passes() -> None:
    ...
    failed = _failed_check(invalid, "trigger_point_before_conflict_and_in_ego_lane")
    request = RepairRequest(
        user_intent=None,
        scenario_spec=invalid,
        failed_check_results=(failed,),
    ...
    repaired = apply_patch(invalid, proposal.patch)
    assert _check(repaired, "trigger_point_before_conflict_and_in_ego_lane").passed is True
...
def test_fake_provider_proposes_timing_lead_time_patch_and_revalidation_passes() -> None:
    ...
    failed = _failed_timing_check(invalid, "ego_lead_time_within_timing_policy")
    request = RepairRequest(
    ...
    repaired = apply_patch(invalid, proposal.patch)
    timing_results = run_pedestrian_occlusion_timing_checks(repaired)
...
def test_fake_provider_declines_unsupported_failed_check() -> None:
    ...
    request = RepairRequest(
        ...
        scenario_spec=spec,
        failed_check_results=(unsupported,),
...
def test_fake_provider_does_not_call_patch_build_runtime_or_artifact_tools(monkeypatch) -> None:
    ...
    request = RepairRequest(
        None,
        invalid,
        (_failed_check(invalid, "parked_van_footprint_in_parking_strip"),),
        ("reposition_actor_to_band",),
    )
...
def _failed_check(spec, name: str) -> CheckResult:
    result = _check(spec, name)
    assert result.passed is False
    return result


def _check(spec, name: str) -> CheckResult:
    return next(result for result in run_pedestrian_occlusion_checks(spec) if result.name == name)


def _failed_timing_check(spec, name: str) -> CheckResult:
    result = next(result for result in run_pedestrian_occlusion_timing_checks(spec) if result.name == name)
    assert result.passed is False
    return result
