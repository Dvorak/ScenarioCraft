...
def execute_prepared_demo_case(
    ...
    orchestrator_result = run_bounded_orchestrator(
        prepared.experiment_spec,
        repair_provider=FakeRepairProvider(),
        output_dir=case_dir,
        scenario_text=_case_user_intent(prepared.case) or "",
        run_runtime=False,
        run_esmini_check=False,
    )
    run_result = orchestrator_result.repair_run_result
    first_round = run_result.rounds[0] if run_result is not None and run_result.rounds else None
    completed_setup_values = {
        **prepared.setup_values,
        **(_repair_outcome_values(prepared.case, run_result) if run_result is not None else {}),
    }
    return DemoCaseExecution(
        ...
        terminal_status=orchestrator_result.terminal_status,
        terminal_reason=orchestrator_result.terminal_reason,
        artifact_paths=tuple(
            orchestrator_result.build_result.artifact_paths()
            if orchestrator_result.build_result is not None
            else ()
        ),
